...
FROM user u LEFT JOIN images i ON u.iduser = i.user_iduser 
...
*/
SELECT u.thelogin, i.theurl, p.thename, a.idarticle, a.thetitle,
GROUP_CONCAT(a.thetitle ORDER BY a.thetitle SEPARATOR '-|-|-') AS titre,
GROUP_CONCAT(a.idarticle ORDER BY a.thetitle) as article
FROM user u 
LEFT JOIN images i ON u.iduser = i.user_iduser 
INNER JOIN permission p ON p.idpermission = u.permission_idpermission
INNER JOIN article a ON a.user_iduser = u.iduser
GROUP BY u.iduser;



SELECT c.thetitle AS titre_categorie,a.thetitle,u.thename,i.theurl,p.thename AS permision_nom,
GROUP_CONCAT(a.idarticle) AS idarticle,
GROUP_CONCAT(a.thetitle separator  '--|||--') AS titre
FROM categorie c
LEFT JOIN categorie_has_article  ON categorie_has_article.categorie_idcategorie = c.idcategorie
LEFT JOIN article a ON categorie_has_article.article_idarticle =a.idarticle
LEFT JOIN user u ON a.user_iduser=u.iduser
LEFT JOIN images i ON i.user_iduser = u.iduser
INNER JOIN permission p ON p.idpermission = u.permission_idpermission
